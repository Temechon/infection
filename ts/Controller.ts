...
class Controller {
    ...
    // If set to true, the controller function 'move' should be called in the render loop manually by the developper
    private _manualModel : boolean = false;
    
    // If set to true, the model will move with collisions. An ellipsoid shoud be enabled on the model
    private _moveWithCollisions : boolean = false;

    constructor(minion : BABYLON.AbstractMesh, options:{manualMode?:boolean, moveWithCollisions?:boolean}) {
        this._minion = minion;

        // Is manual mode ?
        this._manualModel = options.manualMode;
        // Move with collisions ?
        this._moveWithCollisions = options.moveWithCollisions;

        this.findSkeleton();

    
        // Add move function to the character if auto mode
        if (!this._manualModel) {
